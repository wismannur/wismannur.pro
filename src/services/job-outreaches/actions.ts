 ...
 */
export async function uploadOutreachAttachment(
	formData: FormData,
): Promise<JobOutreachAttachment> {
	await assertAdmin();
...
export async function getJobOutreaches(filters?: {
	...
	return rows.map((row) =>
		toJobOutreach(
			row,
			[],
			row.jobApplicationId ? appMap.get(row.jobApplicationId) : null,
		),
	);
}
...
export async function getJobOutreachById(id: string): Promise<JobOutreach | null> {
	...
	return toJobOutreach(
		row,
		messages.map(toJobOutreachMessage),
		jobApp ?? null,
	);
}
...
export async function updateJobOutreach(
	...
	if (data.contactLinkedin !== undefined) updatePayload.contactLinkedin = data.contactLinkedin || null;
	...
	if (data.followUpDueDate !== undefined) updatePayload.followUpDueDate = data.followUpDueDate || null;
	if (data.jobApplicationId !== undefined) updatePayload.jobApplicationId = data.jobApplicationId || null;
...
export async function getOutreachAnalytics(): Promise<OutreachAnalytics> {
	...
	const responseRate =
		sentOrActiveCount > 0
			? Math.round((repliedCount / sentOrActiveCount) * 100)
			: 0;
