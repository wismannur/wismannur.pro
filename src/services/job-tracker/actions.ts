...
import {
  ...
  enrichCompanyIntelligenceWithGemini,
  evaluateMockInterviewAnswerWithGemini,
  generateInboundReachoutReplyWithGemini,
...
const toJobApplication = (
  ...
  companyIntelligence: row.companyIntelligence ?? undefined,
  inboundReachout: row.inboundReachout ?? undefined,
  createdAt: row.createdAt,
  updatedAt: row.updatedAt,
  interviews,
});
...
export async function getAllApplications(): Promise<JobApplication[]> {
  ...
  const db = getDb();
  const appRows = await db.select().from(jobApplications).orderBy(desc(jobApplications.updatedAt));
  ...
  return appRows.map((app) => toJobApplication(app, interviewsByApp.get(app.id) ?? []));
}
...
export async function createApplication(data: NewJobApplication): Promise<string> {
  ...
  const [{ id }] = await db
    ...
    .values({
      ...data,
      appliedAt:
        data.status === "applied" && !data.appliedAt ? new Date() : (data.appliedAt ?? null),
      ...
      companyIntelligence: data.companyIntelligence ?? null,
      inboundReachout: data.inboundReachout ?? null,
    })
...
export async function updateApplicationStatus(
  ...
  await db.update(jobApplications).set(updatePayload).where(eq(jobApplications.id, id));
...
export async function createInterview(data: NewJobInterview): Promise<string> {
  ...
  if (
    app &&
    (app.status === "wishlist" || app.status === "applied" || app.status === "screening")
  ) {
    const targetStatus =
...
export async function updateInterview(id: string, data: UpdateJobInterview): Promise<void> {
  await assertAdmin();
...
export async function getAnalytics(): Promise<JobTrackerAnalytics> {
  ...
  const funnel = [
    ...
    },
    {
      ...
      percentage:
        totalApplications > 0
          ? Math.round(((statusCounts.offering + statusCounts.accepted) / totalApplications) * 100)
          : 0,
    },
  ];
...
export async function aiParseJobPosting(rawContent: string): Promise<ParsedJobPosting> {
  await assertAdmin();
...
export async function aiGenerateInterviewPrep(interviewId: string): Promise<InterviewPrepResult> {
  await assertAdmin();
...
export async function aiEnrichCompanyIntelligence(params: {
  companyName: string;
  ...
  await assertAdmin();
  return enrichCompanyIntelligenceWithGemini(params);
}

export async function aiGenerateInboundReply(params: {
