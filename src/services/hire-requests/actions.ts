...
export async function getHireRequests(page = 1, _startAfterDoc: unknown = null, status?: string) {
  ...
  const db = getDb();
  const where =
    status && status !== "all" ? eq(hireRequests.status, status as HireRequestStatus) : undefined;
...
export async function getById(id: string): Promise<HireRequest | null> {
  await assertAdmin();
  const [row] = await getDb().select().from(hireRequests).where(eq(hireRequests.id, id)).limit(1);
