...
export default function CmsResumePage() {
  ...
  if (
    prevFilters.activeKind !== activeKind ||
    prevFilters.searchQuery !== searchQuery ||
    prevFilters.filterStatus !== filterStatus
  ) {
    setPrevFilters({ activeKind, searchQuery, filterStatus });
    setCurrentPage(1);
  }
  ...
  useRegisterCmsPageContext({
    pageTitle: "Resume Entries",
    summary: `Managing ${stats.expCount} experience and ${stats.eduCount} education entries (${stats.published} published). Viewing ${activeKind} on page ${currentPage}.`,
    activeItems: filteredEntries.map((e) => ({
      id: e.id,
      kind: e.kind,
      title: e.title,
      ...
      <CmsMetricCards
        ...
        metrics={[
          ...
          },
          {
            ...
            icon: GraduationCap,
            color: "purple",
            active: activeKind === "education",
            onClick: () => setActiveKind("education"),
          },
          {
            id: "published",
            label: "Published",
