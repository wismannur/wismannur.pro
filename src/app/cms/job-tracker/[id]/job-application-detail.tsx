...
import { CompanyTimezoneWidget } from "./components/company-timezone-widget";
import { TabCvTailoring } from "./components/tab-cv-tailoring";
import { TabInboundReachout } from "./components/tab-inbound-reachout";
...
export function JobApplicationDetail({ initialId }: { initialId: string }) {
  ...
  const handleDirectUpdateApplication = async (updated: Partial<JobApplication>) => {
    if (!application) return;
    try {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12 animate-fade-in">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="grid grid-cols-2 sm:grid-cols-6 w-full md:w-auto h-auto sm:h-12 p-1 bg-[#0C0E18] border border-white/[0.08] rounded-2xl gap-1">
          <TabsTrigger
            value="inbound"
          ...
          >
            ...
            <span>Inbound / InMail</span>
            {currentApp.inboundReachout?.messageRaw && (
              <Badge
                variant="secondary"
                className="text-[10px] h-4 px-1.5 ml-0.5 bg-blue-500/20 text-blue-300 font-mono"
              >
                InMail
              </Badge>
            )}
        ...
        {/* TAB 0: INBOUND RECRUITER REACHOUT & INMAIL DRAFTER */}
        ...
        {/* TAB 4: OVERVIEW & JOB DESCRIPTION & DOSSIER */}
        <TabsContent value="overview" className="space-y-6">
          <TabJobOverview
            ...
            onDirectUpdate={handleDirectUpdateApplication}
          />
        </TabsContent>
