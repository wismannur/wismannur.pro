...
export function JobApplicationDetail({ initialId }: { initialId: string }) {
	...
	const [pendingOutcomeStatus, setPendingOutcomeStatus] = useState<JobApplicationStatus | null>(null);
	...
	const handleSavePostMortem = async (data: PostMortemData) => {
		...
		try {
			...
			const updatedNotes = (application.notes ? application.notes.trim() + "\n" : "") + reflectionNote;
			...
			refetch();
			toast.success(`Post-mortem saved and status updated to ${JOB_STATUS_CONFIG[targetStatus].label}`);
		} catch (error) {
						...
						<div className="flex items-center gap-2">
							<Button
								...
								className={`h-7 text-[11px] font-semibold border-rose-500/30 text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 ${
									currentApp.status === "rejected" ? "bg-rose-500/15 border-rose-500/50 ring-1 ring-rose-500/30" : ""
								}`}
							>
								❌ Mark Rejected
							</Button>
							<Button
								...
								className={`h-7 text-[11px] font-semibold border-orange-500/30 text-orange-600 dark:text-orange-400 hover:bg-orange-500/10 ${
									currentApp.status === "ghosted" ? "bg-orange-500/15 border-orange-500/50 ring-1 ring-orange-500/30" : ""
								}`}
							>
								👻 Mark Ghosted
							</Button>
						...
						<div className="p-3.5 rounded-xl border border-rose-500/30 bg-rose-500/5 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
							<div className="space-y-0.5">
								...
								<p className="text-[11px] text-muted-foreground">
									Keep track of what went well, interviewer feedback, and improvement areas to refine your career search strategy.
								</p>
							</div>
							<Button
								size="sm"
