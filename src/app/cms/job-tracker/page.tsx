...
export default function JobTrackerPage() {
  ...
  // Expose live rendered applications to CMS Copilot
  useRegisterCmsPageContext(
    useMemo(() => {
      if (filteredApplications.length === 0) return null;
      return {
        pageTitle: "Job Tracker",
        summary: `Currently viewing ${filteredApplications.length} job applications out of ${applications.length} total. View mode: ${viewMode}. Filters: platform=${platformFilter}, status=${statusFilter}, search="${searchQuery}"`,
        filters: { viewMode, platformFilter, statusFilter, searchQuery },
        activeItems: filteredApplications.slice(0, 15).map((a) => ({
          id: a.id,
          ...
          salary: a.salaryMin && a.salaryMax ? `${a.salaryMin} - ${a.salaryMax} ${a.salaryCurrency}` : undefined,
          appliedAt: a.appliedAt,
          notes: a.notes?.slice(0, 120),
          interviewsCount: a.interviews?.length || 0,
        })),
      };
    }, [filteredApplications, applications.length, viewMode, platformFilter, statusFilter, searchQuery])
  );
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-4 items-stretch lg:items-center">
        ...
        >
          <TabsList className="grid grid-cols-3 w-full lg:w-auto p-1 bg-[#0C0E18] border border-white/[0.08] rounded-2xl h-auto">
            <TabsTrigger
              value="kanban"
            ...
            <Select value={platformFilter} onValueChange={setPlatformFilter}>
              ...
              <SelectContent className="bg-[#0C0E18] border-white/[0.08] text-xs">
                ...
                <SelectGroup>
                  ...
                  <SelectItem value="company_website">Company Careers</SelectItem>
