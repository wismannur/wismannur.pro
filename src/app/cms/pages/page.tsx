...
export default function CmsPagesPage() {
  ...
  useRegisterCmsPageContext({
    pageTitle: "Page Copy & Headers",
    summary: `Managing hero copy, section headlines, and CTAs for ${activeEntries.length} key public pages.`,
    activeItems: activeEntries.map((e) => ({
      page: e.page,
      label: PAGE_INFO[e.page]?.label ?? e.page,
      path: PAGE_INFO[e.page]?.path,
  ...
  return (
    <div className="space-y-6">
      <CmsPageHeader
        icon={FileText}
        title="Page Copy"
        description="Edit the hero copy, section headers, SEO meta, and CTA blocks for every active public page."
        badge={activeEntries.length > 0 ? `${activeEntries.length} pages` : undefined}
        onRefresh={() => refetch()}
