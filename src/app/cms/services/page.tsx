...
export default function CmsServicesPage() {
  ...
  // Expose live rendered service requests to CMS Copilot
  useRegisterCmsPageContext(
    useMemo(() => {
      if (filteredRequests.length === 0) return null;
      return {
        pageTitle: "Inbox & Leads - Service Requests",
        summary: `Loaded ${filteredRequests.length} service requests (New: ${stats.new}, In-Progress: ${stats.inProgress}, Completed: ${stats.completed}). Status filter: ${filterStatus || "all"}, search: "${searchQuery}"`,
        filters: { status: filterStatus, query: searchQuery, page: currentPage },
        activeItems: filteredRequests.slice(0, 15).map((r) => ({
          id: r.id,
          name: r.name,
          email: r.email,
          company: r.company,
          ...
          projectDetails: r.projectDetails?.slice(0, 150),
          status: r.status,
          createdAt: r.createdAt,
        })),
      };
    }, [filteredRequests, stats, filterStatus, searchQuery, currentPage])
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <CmsPageHeader
        ...
        description="Manage, review, and respond to incoming service inquiries and custom project proposals."
        onRefresh={() => refetch()}
