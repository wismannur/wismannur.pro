...
export default function CmsHireRequestsPage() {
  ...
  // Expose live rendered hire requests to CMS Copilot
  useRegisterCmsPageContext(
    useMemo(() => {
      if (filteredRequests.length === 0) return null;
      return {
        pageTitle: "Inbox & Leads - Hire Requests",
        summary: `Loaded ${filteredRequests.length} hire requests (New: ${stats.new}, Reviewed: ${stats.reviewed}, Interviewing: ${stats.interviewing}). Filter status: ${filterStatus || "all"}, search: "${searchQuery}"`,
        filters: { status: filterStatus, query: searchQuery, page: currentPage },
        activeItems: filteredRequests.slice(0, 15).map((h) => ({
          id: h.id,
          company: h.company,
          name: h.name,
          email: h.email,
  ...
  return (
    <div className="space-y-6">
      ...
      <CmsPageHeader
        ...
        description="Recruitment messages, full-time offers, and direct hiring proposals from /hire-me."
