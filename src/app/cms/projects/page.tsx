...
export default function CmsProjectsPage() {
  ...
  useRegisterCmsPageContext({
    ...
    summary: `Managing ${stats.total} projects (${stats.published} published, ${stats.featured} featured). Page ${currentPage}.`,
    activeItems: filteredProjects.map((p) => ({
      id: p.id,
      title: p.title,
      slug: p.slug,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3">
        <form onSubmit={handleSearch} className="relative w-full sm:w-80">
          ...
          <Input
            placeholder="Search by title, tech stack, summary..."
            value={searchQuery}
        ...
        <div className="flex items-center gap-2.5">
          <Select value={filterStatus} onValueChange={handleFilterChange}>
            ...
            <SelectContent className="bg-[#0C0E18] border-white/[0.08] text-slate-200">
              <SelectItem value="all" className="text-xs focus:bg-indigo-500/10 focus:text-indigo-300">All Projects</SelectItem>
