...
export default function JobOutreachesPage() {
  ...
  // Expose live rendered outreaches to CMS Copilot
  useRegisterCmsPageContext(
    useMemo(() => {
      if (outreaches.length === 0) return null;
      return {
        pageTitle: "Career Hub - Job Outreaches",
        summary: `Loaded ${outreaches.length} outreach records (Draft: ${analytics?.statusCounts?.draft || 0}, Sent: ${analytics?.awaitingReply || 0}, Replied: ${analytics?.replied || 0}, Converted: ${analytics?.converted || 0}). Tab: ${statusTab}, Type: ${typeFilter}, search: "${searchQuery}"`,
        filters: { status: statusTab, type: typeFilter, query: searchQuery },
        activeItems: outreaches.slice(0, 15).map((o) => ({
          id: o.id,
    ...
    }, [outreaches, analytics, statusTab, typeFilter, searchQuery])
  );
        ...
        <AlertDialogContent className="bg-[#0C0E18] border-white/[0.1] text-slate-200">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-400 text-xs">
              This action will permanently delete the outreach record along with all associated
