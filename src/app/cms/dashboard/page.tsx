...
const Dashboard = () => {
  ...
  useRegisterCmsPageContext(
    useMemo(() => {
      if (!data) return null;
      return {
        pageTitle: "Dashboard & General - Command Center",
        summary: `Dashboard Overview: Blogs (${data.counts.blogs.published}/${data.counts.blogs.total} published), Projects (${data.counts.projects.published}/${data.counts.projects.total} published), Total Views: ${data.counts.totalViews}, Unread Contacts (${data.counts.contacts.unread}), Pending Service Requests (${data.counts.serviceRequests.pending}), Pending Hires (${data.counts.hireRequests.pending}), Active Alerts: ${data.alerts.length}`,
        activeItems: {
          metrics: data.counts,
          recentInbox: data.inbox,
