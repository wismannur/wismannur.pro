...
function extractRecipientEmails(rawTo: unknown): string[] {
  ...
  for (const item of toList) {
    if (typeof item === "string") {
      ...
      const email = String(obj.email || obj.address || "")
        .trim()
        .toLowerCase();
...
function extractIdFromToAddress(toRaw: unknown): {
  ...
  for (const item of toList) {
    ...
    const serviceMatch = str.match(
      /(?:^|\+)?(service(?:[-_]requests?)?[-_][0-9]{10}-[a-z0-9]{3,})/i
    );
    if (serviceMatch && serviceMatch[1]) {
      const normalized = serviceMatch[1]
        .replace(/^service[-_]requests?[-_]/i, "service-")
        .replace("_", "-")
        .toLowerCase();
      return { id: normalized, type: "service_request" };
    }
    const hireMatch = str.match(
      /(?:^|\+)?(hire(?:[-_]inquir(?:y|ies))?[-_][0-9]{10}-[a-z0-9]{3,})/i
    );
    if (hireMatch && hireMatch[1]) {
      const normalized = hireMatch[1]
        .replace(/^hire[-_]inquir(?:y|ies)[-_]/i, "hire-")
        .replace("_", "-")
        .toLowerCase();
      return { id: normalized, type: "hire_request" };
    }
    const generalMatch = str.match(
      /((?:outreach|contact|service|hire)[-_][0-9]{10}-[a-z0-9]{3,})/i
    );
...
function extractInquiryId(subject: string, rawBody?: string): string | null {
  ...
  if (subject) {
    const refMatch = subject.match(
      /(?:\[|\()?\bRef:\s*#?((?:outreach|contact|service|hire)[-_][0-9]{10}-[a-z0-9]{3,}|[a-zA-Z0-9_-]+)(?:\]|\))?/i
    );
    if (refMatch && refMatch[1]) {
      return refMatch[1].trim();
    }
    const generalMatch = subject.match(
      /((?:outreach|contact|service|hire)[-_][0-9]{10}-[a-z0-9]{3,})/i
    );
    if (generalMatch && generalMatch[1]) {
  ...
  if (rawBody) {
    const refMatchBody = rawBody.match(
      /(?:\[|\()?\bRef:\s*#?((?:outreach|contact|service|hire)[-_][0-9]{10}-[a-z0-9]{3,}|[a-zA-Z0-9_-]+)(?:\]|\))?/i
    );
    if (refMatchBody && refMatchBody[1]) {
      return refMatchBody[1].trim();
    }
    const generalMatchBody = rawBody.match(
      /((?:outreach|contact|service|hire)[-_][0-9]{10}-[a-z0-9]{3,})/i
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Fail-closed security guard: In production, webhook secret MUST be configured
    if (process.env.NODE_ENV === "production" && !webhookSecret) {
      console.error(
        "[Resend Inbound] Rejected webhook in production: RESEND_WEBHOOK_SECRET is not configured"
      );
      return NextResponse.json(
        { error: "Webhook endpoint is not configured for production use" },
        { status: 500 }
      );
    }

    // Verify Svix / StandardWebhooks signature
    if (webhookSecret) {
      const svixId =
        req.headers.get("svix-id") || req.headers.get("webhook-id") || req.headers.get("x-svix-id");
      const svixTimestamp =
    ...
    if (!textContent) {
      textContent =
        rawText.trim() || htmlToPlainText(rawHtml) || subject || "(Inbound message without body)";
    }
    ...
    if (!senderEmail) {
      console.warn(
        "[Resend Inbound] Webhook received but sender email is missing. fromRaw:",
        fromRaw
      );
    ...
    const currentEnvDomain = (process.env.RESEND_EMAIL_DOMAIN || "wismannur.pro")
      .trim()
      .toLowerCase();

    if (
      recipientEmails.length > 0 &&
      !isRecipientMatchingDomain(recipientEmails, currentEnvDomain)
    ) {
      console.log(
    ...
    const inboundMessageId =
      (typeof emailHeaders["message-id"] === "string"
        ? (emailHeaders["message-id"] as string)
        : undefined) ||
      (typeof emailHeaders["Message-ID"] === "string"
        ? (emailHeaders["Message-ID"] as string)
        : undefined) ||
    ...
    const rawExplicitRefId =
      toInfo.id || headerRefId || extractInquiryId(subject, rawText || rawHtml);
    ...
    const referencedMessageIds = (function () {
      const inReplyTo = String(
        emailHeaders["in-reply-to"] || emailHeaders["In-Reply-To"] || fullEmail?.in_reply_to || ""
      );
      const refs = String(
        emailHeaders["references"] || emailHeaders["References"] || fullEmail?.references || ""
      );
      ...
      if (bracketMatches && bracketMatches.length > 0) {
        return bracketMatches
          .map((b) => b.replace(/[<>]/g, "").trim().toLowerCase())
          .filter(Boolean);
      }
      return combined
        .split(/\s+/)
      ...
      for (const refId of referencedMessageIds) {
        ...
        const [matchedInquiryMsg] = await db
          .select({
            ...
            inquiryType: inquiryMessages.inquiryType,
          })
    ...
    if (!targetInquiryId && senderEmail && isReplyOrFwd) {
      ...
      const [latestOutreachRows, latestContactRows, latestServiceRows, latestHireRows] =
        await Promise.all([
          db
            .select()
            .from(jobOutreaches)
    ...
    if (targetInquiryType === "contact") {
      await db.update(contacts).set({ status: "new" }).where(eq(contacts.id, targetInquiryId));
