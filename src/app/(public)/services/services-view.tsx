...
import { ArrowRight, CheckCircle2, ChevronDown, GripHorizontal, HelpCircle, Sparkles } from "lucide-react";
...
export function ServicesView({
  ...
  // Resizable Project Scope Textarea
  const [projectDetailsHeight, setProjectDetailsHeight] = useState<number>(140);
  const isDraggingProjectDetailsRef = useRef(false);
  const dragStartYRef = useRef(0);
  const startHeightRef = useRef(140);

  const handleMouseDownOnResize = (e: React.MouseEvent) => {
    e.preventDefault();
    isDraggingProjectDetailsRef.current = true;
    dragStartYRef.current = e.clientY;
    startHeightRef.current = projectDetailsHeight;

    const handleMouseMove = (moveEvent: MouseEvent) => {
      if (!isDraggingProjectDetailsRef.current) return;
      const deltaY = moveEvent.clientY - dragStartYRef.current;
      const maxHeight = Math.min(650, Math.floor(window.innerHeight * 0.75));
      const newHeight = Math.min(Math.max(startHeightRef.current + deltaY, 100), maxHeight);
      setProjectDetailsHeight(newHeight);
    };

    const handleMouseUp = () => {
      isDraggingProjectDetailsRef.current = false;
  ...
  const handleTouchStartOnResize = (e: React.TouchEvent) => {
    ...
    const handleTouchMove = (moveEvent: TouchEvent) => {
      if (!isDraggingProjectDetailsRef.current || moveEvent.touches.length !== 1) return;
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] text-gray-500 hidden sm:flex items-center gap-1 font-mono">
                            <GripHorizontal className="h-3 w-3 opacity-60" /> Drag to resize
                          </span>
                        </div>
                        <FormControl>
                          <div className="relative flex flex-col rounded-xl border border-white/[0.1] bg-black/40 overflow-hidden focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/40 transition-all shadow-inner">
                            <Textarea
                              ...
                              style={{ height: `${projectDetailsHeight}px` }}
                              className="w-full text-xs sm:text-sm leading-relaxed resize-none rounded-none border-0 bg-transparent text-white placeholder:text-gray-500 focus-visible:ring-0 focus-visible:ring-offset-0 px-3.5 py-3 overflow-y-auto scrollbar-thin"
                              {...field}
                            />
                            ...
                            <div
                              ...
                              className="group w-full h-3.5 cursor-row-resize flex items-center justify-center bg-white/[0.02] hover:bg-white/[0.06] active:bg-primary/20 transition-colors select-none border-t border-white/[0.06]"
