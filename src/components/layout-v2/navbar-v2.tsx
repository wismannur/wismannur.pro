...
export function NavbarV2({ enableBlog = true }: NavbarV2Props) {
  ...
  // Prevent background scroll & handle Escape key when mobile drawer is open
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <header className="fixed top-3 sm:top-5 inset-x-0 z-50 flex justify-center px-3 sm:px-4 pointer-events-none">
      ...
      >
        ...
        <nav className="hidden lg:flex items-center gap-1 bg-white/[0.03] border border-white/[0.06] rounded-full p-1 px-1.5">
          {navLinks.map((link) => {
      ...
      {isOpen && (
        <div
          ...
          className="pointer-events-auto fixed inset-0 top-0 z-40 bg-black/90 backdrop-blur-2xl flex flex-col justify-between p-6 pt-24 animate-fade-in lg:hidden overflow-y-auto"
        >
          <nav className="flex flex-col items-center justify-center space-y-3 flex-1 py-4">
            {navLinks.map((link) => {
              const isActive = isLinkActive(link.path);
              return (
                <Link
              ...
              {user ? (
                ...
                >
                  <Link
                    href="/cms/dashboard"
                    onClick={() => setIsOpen(false)}
                    data-umami-event="navbar-v2-mobile-dashboard-click"
                  >
                ...
                >
                  <Link
                    href="/hire-me"
                    onClick={() => setIsOpen(false)}
                    data-umami-event="navbar-v2-mobile-hire-click"
                  >
